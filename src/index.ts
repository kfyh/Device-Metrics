...
const DeviceDetectorStub = {
  parse(ua: string) {
    const result: any = { client: {}, os: {}, device: {}, bot: null };
...
async function inspectDevice(): Promise<DeviceMetric> {
  // UA
  ...
  const gl = (gl2 || gl1) as WebGLRenderingContext;

  let unmaskedVendor: string | null = null, unmaskedRenderer: string | null = null, vendor: string | null = null, renderer: string | null = null;
  ...
  const gp = (param: number): number | null => { try { const v = gl?.getParameter(param); return typeof v === "number" ? v : null; } catch { return null; } };
  const hasExt = (name: string): boolean => { try { return !!gl?.getExtension(name); } catch { return false; } };

  // WebGPU
  let webgpu: DeviceMetric['gpu']['webgpu'] = {
    supported: false,
  ...
  const gpu = {
    ...
    maxTextureSize: gp(gl?.MAX_TEXTURE_SIZE ?? 0),
    maxCubeMapTextureSize: gp(gl?.MAX_CUBE_MAP_TEXTURE_SIZE ?? 0),
    maxRenderBufferSize: gp(gl?.MAX_RENDERBUFFER_SIZE ?? 0),
    maxTextureImageUnits: gp(gl?.MAX_TEXTURE_IMAGE_UNITS ?? 0),
    maxVertexAttribs: gp(gl?.MAX_VERTEX_ATTRIBS ?? 0),
    maxVaryingVectors: gp(gl?.MAX_VARYING_VECTORS ?? 0),
    maxFragmentUniformVectors: gp(gl?.MAX_FRAGMENT_UNIFORM_VECTORS ?? 0),
    maxVertexUniformVectors: gp(gl?.MAX_VERTEX_UNIFORM_VECTORS ?? 0),
    compressedTextures: {
